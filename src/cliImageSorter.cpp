...
CliImageSorter::CliImageSorter(const std::string &filename)
...
CliImageSorter::CliImageSorter(SDL_Surface *surface, int n_channels)
    : m_image((uint8_t *)surface->pixels), m_width(surface->w),
      m_height(surface->h), m_channels(n_channels), m_destroy_image(false)
{
}

CliImageSorter::~CliImageSorter()
{
  if (m_destroy_image && m_image)
    stbi_image_free(m_image);
}

void CliImageSorter::sort_column(int column_index, int start, int end,
...
void CliImageSorter::sort_vertical(int hue_value)
{
...
void CliImageSorter::write_to_file(const std::string &filename)
{
